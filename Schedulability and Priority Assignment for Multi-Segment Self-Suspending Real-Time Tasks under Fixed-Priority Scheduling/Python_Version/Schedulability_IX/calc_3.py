...
def deadline_monotonic(task_, n_, m_):
    for i in range(n_):
        for j in range(m_ - 1):
            task_[i][2 * j] += task_[i][2 * j + 1]
            task_[i][2 * j + 1] = 0


def bubble_sort(task_, n_):
    for i in range(n_):
...
print("=========================================================")
print(task)
deadline_monotonic(task, n, M[0])
bubble_sort(task, n)
print(task)
flag_all_passed = 1
temp = 0
for i in range(n):
    temp += execution_time(task, i) / T[i]
temp = temp / num_cpu
# print("temp_atFirst = ", temp)
for i in range(n):
    max_workload = 0
    for j in range(i + 1, n):
        toJudge = (execution_time(task, j) / T[i]) / num_cpu
        if toJudge > max_workload:
            max_workload = toJudge
    temp += max_workload
    # print("For task ", i, ", max wkld function = ", max_workload)
upper_bound = n * (2 ** (1 / n) - 1)
# print("temp_atLast = ", temp)
...
# n_g1 = len(task_g1)
# n_g2 = len(task_g2)
# bubble_sort(task_g1, n_g1)
# bubble_sort(task_g2, n_g2)
# print("=========================================================")
# for i in range(n_g1):
