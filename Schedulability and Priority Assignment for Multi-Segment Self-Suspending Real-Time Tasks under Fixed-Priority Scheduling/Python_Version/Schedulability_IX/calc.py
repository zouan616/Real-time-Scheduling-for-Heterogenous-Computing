...
def feasible(task_, i, j, t, d, num_cpu):
...
def PriorityAssignment(task_, n_, t, d, num_cpu):
    task_cpy = task_
    len_ = n
    for j in range(n_ - 1, 0, -1):
        unassigned = True
        for i in range(len_):
            if feasible(task_cpy, i, j, t, d, num_cpu):
                # print("task = ", task_cpy)
                swap(task_cpy, i, j, t, d)
...
temp = PriorityAssignment(task, n, T, D, num_cpu)
if temp == 0:
    print("All tasks passed")
else:
    print("Failed")
